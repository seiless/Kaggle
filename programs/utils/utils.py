...
def evaluate_null_int_columns_prediction_model(
    df: pd.DataFrame, column: str, features: list
) -> None:
    """
...
def evaluate_null_categorical_column_prediction_model(
    df: pd.DataFrame, column: str, features: list
) -> None:
    """
...
def plot_binary_ratio_by_category(
    ...
    count_df["ratio"] = count_df.groupby(category_col)["count"].transform(
        lambda x: x / x.sum()
    )
    plot_df = count_df.pivot(
        index=category_col, columns=target_col, values="ratio"
    ).fillna(0)
    ...
    ax = plot_df.plot(
        kind="barh", stacked=True, color=["lightgray", "steelblue"], figsize=(12, 8)
    )
...
def cluster_titles_by_survival(
    df: pd.DataFrame, category_col: str, target_col: str, k_range: range = range(2, 6)
) -> dict:
    """
...
def evaluate_null_categorical_column_prediction_model_rf(
    df: pd.DataFrame, category_col: str, features: list
) -> None:
    """
    ...
    for col in features:
        if (
            ...
            or df_encoded[col].dtype.name == "category"
        ):
            le = LabelEncoder()
